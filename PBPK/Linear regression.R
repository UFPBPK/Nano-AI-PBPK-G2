...
#PBPK NP parameters
Theta<- read.csv(file='AI-PBPK-Parameter.csv',fileEncoding = 'UTF-8-BOM')
...
TvsC_Dat$ID<-na.locf(TvsC_Dat$ID) # replace na values with last no-na value
colnames(TvsC_Dat) <- c("ID","Time", "DETumor")
#Calculate the last sample time point
df_last_time <-TvsC_Dat %>%group_by(ID) %>%filter(Time == max(Time)) %>%ungroup()
...
Cost_nano <- function(ID, Physic_pars, w = "mean") {
  ...
  timepoint <- df_last_time %>% filter(ID == !!ID) %>% pull(Time)
...
for (id in ID_list) {
  ...
  result <- Cost_nano(ID = id, Physic_pars = Physic_pars, w = "mean")
...
lm_fit <- lm(obs ~ mod, data = all_residuals_df)
summary(lm_fit)
